-- Migration 0002_core.sql: Core schema for Supabase integration with Asia/Jakarta GMT+7
create schema if not exists auth;

-- Stub for local non-Supabase environments
...
-- Policies:
create policy "profiles readable by public" on public.profiles
...
create policy "own completions read" on public.completions
  for select using (auth.uid() = user_id);

create policy "own claimed quests read" on public.claimed_quests
  for select using (auth.uid() = user_id);

create policy "raffles readable" on public.raffles
  for select using (true);

create policy "winners readable" on public.raffle_winners
  for select using (true);

create policy "own entries read" on public.raffle_entries
  for select using (auth.uid() = user_id);

create policy "own ledger read" on public.ledger
  for select using (auth.uid() = user_id);

-- Trigger to initialize profiles and progress on new user signup
