...
const PARTICLES: Partial<Record<MascotMood, { ch: string; className: string; delay: string }[]>> = {
  ...
  sleep: [
    ...
    { ch: "Z", className: "right-[8%] top-[2%] text-[0.7em] font-black text-sky", delay: "0.8s" },
  ],
  think: [{ ch: "?", className: "right-[12%] top-[8%] text-[0.7em] font-black text-fg", delay: "0s" }],
  proud: [{ ch: "✦", className: "right-[14%] top-[10%] text-[0.55em] text-gold", delay: "0.2s" }],
};
    ...
    <div className={cn("relative", className)}>
      {tail === "left" ? (
        <span
          aria-hidden
          className={cn(
            "absolute size-3.5 rotate-45 border-b-2 border-l-2 border-line-strong bg-white z-10",
        ...
        <span
          aria-hidden
          className="absolute -top-2 left-1/2 size-3.5 -translate-x-1/2 rotate-45 border-l-2 border-t-2 border-line-strong bg-white z-10"
        />
      )}
      <div
        className={cn(
          "rounded-[18px] border-2 border-line-strong bg-white text-left font-extrabold text-ink-900 shadow-[0_4px_0_#C8DBF0]",
